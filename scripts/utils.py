...
def configure_runtime_environment(
	...
	if respect_existing:
		os.environ.setdefault("XLA_PYTHON_CLIENT_MEM_FRACTION", default_mem_fraction)
	else:
		os.environ["XLA_PYTHON_CLIENT_MEM_FRACTION"] = default_mem_fraction
...
def load_simulation_dataset(
	...
	if mol in ("cath_full", "cath_quarter", "cath_test"):
		...
		cache_candidates = [f"/ds/project/franz/Datasets/CATH_{mol.split('_', 1)[1]}_{map_name}.npz"]
...
def prepare_dataset_bundle(
	...
	cg_map: str | None,
	*,
	use_position_dataset: bool,
) -> dict[str, Any]:
	"""Prepare model-specific dataset bundle used by training or simulation."""
	from jax_md import space

	model_type = str(model_type).upper()
	box = getattr(data_obj, "box", None)

	if model_type == "AT":
		if hasattr(data_obj, "load_traj"):
			data_obj.load_traj()

		dataset_attr = "dataset_X" if use_position_dataset else "dataset_U"
		dataset_data = getattr(data_obj, dataset_attr)
		displacement_fn = getattr(data_obj, "displacement_fn_X", None)
		species = data_obj.species
		masses = getattr(data_obj, "masses", None)
		n_species = data_obj.n_species

	elif model_type == "CG":
		if not cg_map:
			raise ValueError("cg_map must be provided for CG mode.")
		data_obj.coarse_grain(map=cg_map)

		dataset_attr = "cg_dataset_X" if use_position_dataset else "cg_dataset_U"
		dataset_data = getattr(data_obj, dataset_attr)
		displacement_fn = getattr(data_obj, "displacement_fn_X", None)
		species = data_obj.cg_species
		masses = getattr(data_obj, "cg_masses", None)
		n_species = data_obj.n_cg_species
	else:
		raise ValueError("Invalid model type. Use 'AT' or 'CG'.")
...
def build_mace_config(source: dict[str, Any], use_so3: bool) -> dict[str, Any]:
	...
	return {
		...
		"hidden_irreps": source.get("hidden_irreps", DEFAULT_MACE_CONFIG["hidden_irreps"]),
...
def init_mace_model_and_template(
	...
	*,
	...
	species_default = jnp.asarray(species_default)
	...
	def energy_fn_template(params):
		...
		def energy_fn(position, neighbor, **kwargs):
			species = kwargs.pop("species", species_default)
			mask = kwargs.pop("mask", jnp.ones(position.shape[0], dtype=jnp.bool_))

			pots = gnn_energy_fn(vars_, position, neighbor, species=species, **kwargs)
			if pots.ndim == 2 and pots.shape[-1] == 1:
				pots = pots.squeeze(-1)

			atomic_numbers = jnp.asarray(model_config["atomic_numbers"], dtype=jnp.int32)
			atomic_energies = jnp.asarray(
				model_config["atomic_energies"], dtype=jnp.float32
			)
			mapped_species = jnp.argmax(
				species[:, None] == atomic_numbers[None, :], axis=-1
			)
			return jnp.sum((pots - atomic_energies[mapped_species]) * mask)
