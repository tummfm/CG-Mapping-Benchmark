...
def radius_of_gyration_vectorized(coords: jnp.ndarray, displacement_fn: Callable[[jnp.ndarray, jnp.ndarray], jnp.ndarray]) -> jnp.ndarray:
    """
    ...
    center_of_mass = jnp.mean(corrected_positions, axis=1, keepdims=True)  # Shape: (n_frames, 1, 3)
...
def helicity_vectorized(coords: jnp.ndarray, displacement_fn: Callable[[jnp.ndarray, jnp.ndarray], jnp.ndarray]) -> jnp.ndarray:
    """
...
def plot_helicity_gyration(coords, displacement, starting_frames=None, save_pdf=False, prefix='', suffix='', scale_used=None):
    ...
    print(f"Valid coords shape: {coords_valid.shape}, Number of valid frames: {coords_valid.shape[0]}")
    ...
    if starting_frames is not None:
        xi_norm_ref_starting_frames = xi_norm_vectorized(starting_frames, displacement).flatten()
        helicity_values_starting_frames = helicity_vectorized(starting_frames, displacement)
        rg_values_starting_frames = radius_of_gyration_vectorized(starting_frames, displacement)
    ...
    print(f"Frame with max helicity: {max_idx}, value: {helicity_values[max_helicity_idx_in_valid]}")
    print(f"Frame with min helicity: {min_idx}, value: {helicity_values[min_helicity_idx_in_valid]}")
    ...
    min_sum_idx = int(valid_indices[min_sum_idx_in_valid])  # Convert to original frame index

    print(f"Frame with lowest rg + helicity: {min_sum_idx}, value: {sum_rg_hel[min_sum_idx_in_valid]} "
    ...
    ax1.plot(np.asarray(valid_indices), helicity_values, label='Helicity Content', color='blue', linewidth=line_width)
    ax1.set_ylabel('Helicity Content (Q_hel)')
    ax1.set_xlabel('Frame')
    ax2 = ax1.twinx()
    ax2.plot(np.asarray(valid_indices), rg_values, label='Radius of Gyration', color='orange', linewidth=line_width)
    ...
    utils.plot_histogram_free_energy(
        ax, np.asarray(rg_values), np.asarray(helicity_values),
        kbt=300. * quantity.kb,
        is_angular=False,
        xlabel="$Rg (nm)$",
        ylabel_text="$Q_{hel}$",
    ...
    # Add scatter points
    ax.scatter(rg_values[min_sum_idx_in_valid], helicity_values[min_sum_idx_in_valid],
               color='black', s=60, label='min(Rg+Helicity)', zorder=5)
    ax.scatter(rg_values[max_helicity_idx_in_valid], helicity_values[max_helicity_idx_in_valid],
               color='magenta', s=60, marker='x', label='max Helicity', zorder=5)

    if starting_frames is not None:
        ax.scatter(np.asarray(rg_values_starting_frames).ravel(), np.asarray(helicity_values_starting_frames).ravel(),
                   color='green', s=40, marker='x', label='starting frames', zorder=5)
    ...
    fig, ax = plt.subplots(figsize=(8, 5))
    utils.plot_histogram_free_energy(
        ax, xi_norm_ref_np, helicity_values_np,
        kbt=300. * quantity.kb,
        is_angular=False,
        xlabel="$\\chi_{hel}$",
        ylabel_text="$Q_{hel}$",
    ...
    if starting_frames is not None and xi_norm_ref_starting_frames is not None:
        ax.scatter(np.asarray(xi_norm_ref_starting_frames).ravel(), np.asarray(helicity_values_starting_frames).ravel(),
                   color='green', s=40, marker='x', label='starting frames', zorder=5)
...
def _calculate_xi_norm_single_frame(coords_f: jnp.ndarray, displacement_fn: Callable[[jnp.ndarray, jnp.ndarray], jnp.ndarray]) -> jnp.ndarray:
    """
    Calculates the normalized helical chirality index (xi_norm) for a single frame.
    This corresponds to the normalized version of chi_total (Equation 2 in the paper).
    """
    n_atoms, _ = coords_f.shape
    
    # Needs at least 4 atoms to form one triplet (v1, v2, v3)
    if n_atoms < 4:
        # Return 0.0 for frames too short to calculate handedness
        return jnp.array(0.0) 

    # 1. Calculate vectors v_i = C_alpha(i+1) - C_alpha(i)
    # v_i shape: (n_atoms - 1, 3)
    v_i = vmap(displacement_fn)(coords_f[1:], coords_f[:-1])
    
    # 2. Extract consecutive vector triplets for the sum: (v_i, v_{i+1}, v_{i+2})
    # i runs from 0 to n_atoms - 4. Number of triplets = n_atoms - 3
    v1 = v_i[:-2]    # v_i
    v2 = v_i[1:-1]   # v_{i+1}
    v3 = v_i[2:]     # v_{i+2}
    
    # 3. Calculate the cross product: v_cross = v_i x v_{i+1}
    # cross_product shape: (n_triplets, 3)
    cross_product = jnp.cross(v2, v3)
    
    # 4. Calculate the mixed product (scalar triple product): (v_i x v_{i+1}) . v_{i+2}
    # The dot product is equivalent to jnp.sum(A * B, axis=1)
    # mixed_products shape: (n_triplets,)
    mixed_products = jnp.sum(cross_product * v1, axis=1)
    
    # 5. Sum all mixed products (chi_total)
    chi_total = jnp.sum(mixed_products)
    
    # 6. Normalize by the number of triplets to get the normalized index (xi_norm)
    n_triplets = n_atoms - 3
    xi_norm_value = chi_total / n_triplets
    
    return xi_norm_value

def xi_norm_vectorized(coords: jnp.ndarray, displacement_fn: Callable[[jnp.ndarray, jnp.ndarray], jnp.ndarray]) -> jnp.ndarray:
    """
    Calculates the normalized helical chirality index (xi_norm) for a trajectory.
    This metric measures the average handedness contribution per C_alpha triplet.
    ...
    """
    # Use vmap to apply the single-frame calculation function over all frames.
    # We are mapping over the 0th axis (n_frames) of the input coords.
    xi_norm_values = vmap(_calculate_xi_norm_single_frame, in_axes=(0, None))(coords, displacement_fn)
    
    # Reshape to (n_frames, 1) as requested by the user
...
def plot_1d_dihedral(
    ...
    color = [color_ref, color_mace, color_fm, '#DDCC77', '#CC6677', '#66CC99', '#FF6B6B', '#4A90E2', '#50514F', '#F4A261']

    n_models = len(angles)
    for i in range(n_models):
        if degrees:
            # Convert angles to [-180, 180] range
            angles_conv = ((angles[i] + 180) % 360) - 180
            hist_range = [-180, 180]
        else:
            # Convert angles to [-π, π] range
            angles_conv = ((angles[i] + np.pi) % (2 * np.pi)) - np.pi
            hist_range = [-np.pi, np.pi]

        hist, x_bins = np.histogram(angles_conv, bins=bins, density=True, range=hist_range)
        width = x_bins[1] - x_bins[0]
        bin_center = x_bins[:-1] + width / 2

        ax.plot(
            bin_center, hist, label=labels[i], color=color[i % len(color)],
...
def plot_1d_angle(
    ...
    color = [
        color_ref, color_mace, color_fm, '#DDCC77', '#CC6677',
        '#66CC99', '#FF6B6B', '#4A90E2', '#50514F', '#F4A261'
    ]
...
def plot_1d_bond(
    ...
    # assumes color_ref, color_mace, color_fm, line_width, axis_label_font_size,
    # legend_font_size, tick_font_size are defined elsewhere
    colors = [color_ref, color_mace, color_fm, '#FFB347', '#7851A9', '#66CC99',
              '#FF6B6B', '#4A90E2', '#F4A261']

    # Clean data (drop NaNs) and skip empties
    clean_bonds, clean_labels = [], []
    for arr, lab in zip(bonds, labels):
        a = np.asarray(arr)
        a = a[np.isfinite(a)]
        if a.size > 0:
            clean_bonds.append(a)
            clean_labels.append(lab)
    if not clean_bonds:
        raise ValueError("All provided bond arrays are empty or non-finite.")

    # Shared bin edges across all datasets
    gmin = min(a.min() for a in clean_bonds)
    gmax = max(a.max() for a in clean_bonds)
    ...
    # Plot each as a PMF (bin fractions) so heights sum to 1
    for i, (bond_data, lab) in enumerate(zip(clean_bonds, clean_labels)):
        counts, _ = np.histogram(bond_data, bins=bin_edges)   # <- counts, not density
        total = counts.sum()
        if total == 0:
            continue
        frac = counts / total                                  # <- fractions per bin
        ax.plot(
            bin_centers,
            frac,
            label=lab,
    ...
    # X ticks in tick_bin steps over the combined range
...
def calculate_rdf(
    ...
    if len(bead_types) != sites_per_mol:
        raise ValueError(f"bead_types length ({len(bead_types)}) must match sites_per_mol ({sites_per_mol})")
    
    # Get unique bead types and create all combinations
    unique_types = sorted(list(set(bead_types)))
    bead_combinations = [(t1, t2) for i, t1 in enumerate(unique_types) for t2 in unique_types[i:]]
    ...
    for type1, type2 in bead_combinations:
        if type1 == type2:
            type_mask = (full_bead_types[i_inter] == type1) & (full_bead_types[j_inter] == type1)
        else:
            type_mask = ((full_bead_types[i_inter] == type1) & (full_bead_types[j_inter] == type2)) | \
                       ((full_bead_types[i_inter] == type2) & (full_bead_types[j_inter] == type1))
...
def plot_rdf(
    ...
    color = [
        color_ref, color_mace, color_fm,
        '#DDCC77', '#CC6677', '#66CC99',
        '#FF6B6B', '#4A90E2', '#50514F', '#F4A261'
    ]
...
def calculate_rdf_mse_from_dict(rdf_data, bead_combo, traj_idx1, traj_idx2, 
...
def plot_1d_rdf(
    ...
    """
    Plot RDF data on a single axis for a specific bead combination.
    
    Args:
        ax: Matplotlib axis object
        rdf_data: Dictionary with structure {bead_combo: {traj_idx: (r, g_r)}}
        bead_combo: Tuple specifying the bead combination (e.g., ('A', 'B'))
        labels: List of trajectory labels
        xlabel: X-axis label
        ylabel: Whether to set y-axis label
        tick_spacing: Spacing for x-axis ticks
    ...
    # Check if this bead combination exists in the data
    if bead_combo not in rdf_data:
        print(f"Warning: Bead combination {bead_combo} not found in RDF data")
        return ax
    
    # Plot each trajectory's RDF data
    for traj_idx in rdf_data[bead_combo]:
        if traj_idx < len(labels):
            r, g_r = rdf_data[bead_combo][traj_idx]
            ax.plot(r, g_r, label=labels[traj_idx], 
                   color=color[traj_idx % len(color)], linewidth=line_width)
    ...
    # Set x-ticks based on tick_spacing
    if xlim:
        min_r, max_r = xlim
    else:
        # Get range from data
        all_r = []
        for traj_idx in rdf_data[bead_combo]:
            r, _ = rdf_data[bead_combo][traj_idx]
            all_r.extend(r)
        if all_r:
            min_r = min(all_r)
            max_r = max(all_r)
    ...
    ticks = np.arange(np.floor(min_r / tick_spacing) * tick_spacing, 
                      np.ceil(max_r / tick_spacing) * tick_spacing + tick_spacing, 
                      tick_spacing)
    ax.set_xticks(ticks)
    # Set y-ticks with at most one decimal
